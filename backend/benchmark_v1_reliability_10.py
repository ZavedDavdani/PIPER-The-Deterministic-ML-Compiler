"""
PIPER V1 — 10-Run Reliability Benchmark Runner (Measurement Only)
Authoritative sequential execution of 10 Titanic trials on frozen V1.
"""
...
def run_single_trial(trial_idx: int, is_cold: bool, dataset_path: str, target_column: str = "Survived") -> dict:
    print(f"\n========================================================")
    print(f"  STARTING TRIAL {trial_idx}/10 ({'COLD' if is_cold else 'WARM'})")
    print(f"========================================================")
    ...
    dataset_id = f"titanic_trial_{trial_idx}"
    ...
    # 3. Provider with instrumentation
    ...
    run_id = f"v1_rel_run_{trial_idx}"
    ...
    # 10. Qualitative plan inspections (for successful trials or valid plans)
    qualitative = {
    ...
    trial_result = {
        ...
        "total_planning_latency": sum(a["latency_seconds"] for a in attempts_detail),
        ...
        "new_invalidation": False,
        "budget_exhaustion": budget_exhaustion,
        "parse_failure": parse_failure,
        "provider_failure": provider_failure,
        "timeout": timeout_occurred,
        "schema_violation_count": total_schema_violations,
        "tool_argument_violations": tool_argument_violations,
        "adequacy_violations": adequacy_violations,
    ...
    print(f"Trial {trial_idx} Finished in {total_wall_time:.2f}s:")
...
def main():
    print("=================================================================")
    print("  PIPER V1 — 10-RUN RELIABILITY BENCHMARK (TITANIC / qwen3:4b)")
    print("=================================================================")
    ...
    results_dir = os.path.join("benchmark_results", "v1_reliability_10")
    os.makedirs(results_dir, exist_ok=True)
    raw_results_file = os.path.join(results_dir, "v1_reliability_10.json")
    summary_file = os.path.join(results_dir, "v1_reliability_10_summary.json")
    ...
    for trial_idx in range(start_trial, 11):
        is_cold = (trial_idx == 1)
    ...
    n = len(trials_data)
    successful_trials = sum(1 for t in trials_data if t["completed_successfully"])
    ...
    replan_successful = sum(1 for t in trials_data if t["replan_count"] > 0 and t["completed_successfully"])
    ...
    schema_failure_trials = sum(1 for t in trials_data if t["failure_category"] == "SCHEMA_FAILURE")
    adequacy_failure_trials = sum(1 for t in trials_data if t["failure_category"] == "ADEQUACY_FAILURE")
    ...
    planning_latencies = [t["total_planning_latency"] for t in trials_data]
    ...
    # Decision rule
    classification = ""
    if successful_trials >= 9:
        classification = "HIGHLY RELIABLE"
        decision = "V1 RELIABILITY ACCEPTED FOR DEMO"
    elif successful_trials >= 7:
        classification = "DEMO-USABLE"
        decision = "V1 DEMO-USABLE WITH DOCUMENTED LIMITATIONS"
    elif successful_trials >= 5:
        classification = "UNRELIABLE"
        decision = "V1 RELIABILITY NOT ACCEPTED"
    else:
    ...
    print("\n=================================================================")
    print("  10-RUN RELIABILITY BENCHMARK COMPLETE")
    print("=================================================================")
